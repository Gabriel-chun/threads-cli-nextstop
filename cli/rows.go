...
func searchRow(r *threads.SearchResult) Row {
	return Row{
		Cols:  []string{"id", "username", "score", "tier", "source_queries", "retrieval_sources", "matched_terms", "text", "media", "permalink"},
		Vals:  []string{r.ID, r.Username, strconv.Itoa(r.RelevanceScore), r.RelevanceTier, strings.Join(r.SourceQueries, ","), strings.Join(r.RetrievalSources, ","), strings.Join(r.MatchedTerms, ","), truncate(r.Text, 80), r.MediaType, r.Permalink},
		Value: r,
	}
}
